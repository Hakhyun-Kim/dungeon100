...
export function useDemoDriver(running: boolean, actions: DemoActions) {
  ...
  useEffect(() => {
    ...
    const OVERLAYS = ['lore', 'memory', 'trace', 'quiz', 'draft', 'homedoor', 'altar', 'secretdoor', 'rift', 'collapse', 'memfull', 'arenaover', 'over'];
    ...
    const clickOverlay = () => {
      ...
      const declinePhases = ['homedoor', 'altar', 'secretdoor', 'rift', 'collapse'];
