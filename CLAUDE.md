...
1. **밸런스 시뮬레이터 보스전 봇 개선** — 1차 완성(`src/dev/simBot.ts`, `__d100sim.start({runs:N})` — 사망 층 분포 리포트, 하드런 6·9층 사망 실측). 잔여: 보스전에서 봇이 교착(원거리 링은 벽·어그로 경계 스테일메이트, 근접 링 검증 중) — 보스 층 돌파 로직 튜닝. **2026-07-19 업그레이드 16종·방 이벤트 추가 후 사망 층 분포 재실측 필요** (봇은 이벤트를 '그만둔다'로 거절해 측정 조건 유지). 숨김 탭 장시간 세션은 환경이 열화되니 **보이는 탭에서 돌리는 게 안정적**.
