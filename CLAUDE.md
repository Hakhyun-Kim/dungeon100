...
- **자동 시연 모드: `?demo` 쿼리** (프로덕션에서도 동작 — 예: …/dungeon100/?demo) — 타이틀에 "🎬 자동 시연 보기(약 90초)" 버튼이 생기고, 클릭 한 번이면 게임이 스스로 쇼케이스 투어를 진행: 마을 산책 → 1층 전투 → 보물 3연속(빌드 시각화) → 두 문 달리기 → 몬스터 아레나 → 45·75층(테마·안개) → 85층 폐허 마을 → 30층 보스 → 예고 자막 → 끝 화면(다시 보기/직접 플레이). App 내부 드라이버가 합성 키보드 이벤트(useMoveInput/useSteer가 window 키 이벤트 기반)로 걷고, 장면 전환은 goVillage/enterDungeon/debugJump/debugGrantRef 직접 호출 — DEV 훅 불사용이라 배포판에서도 동작. 오버레이(로어·기억·포털 등)는 1.5초 보여주고 자동으로 넘김(포털·마을 문은 거절). 자막 `.demo-caption`, 종료 버튼, 끝 화면 `.demo-end`. 심사자용 "영상 대체" 링크로 활용.
