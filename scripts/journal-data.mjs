...
// ⚠️ 날짜별 Map으로 모으고 날짜순 정렬한다 — git log 순서는 작성일과 어긋날 수 있어서
//    (CI 자동 커밋·머지 등) 인접 비교만 하면 같은 날이 두 줄로 갈라져 차트가 뒤엉킨다.
const byDayMap = new Map();
for (const c of commits) {
  const d = byDayMap.get(c.date);
  if (d) {
    d.commits++;
    d.ins += c.ins;
    d.del += c.del;
  } else {
    byDayMap.set(c.date, { date: c.date, commits: 1, ins: c.ins, del: c.del });
  }
}
