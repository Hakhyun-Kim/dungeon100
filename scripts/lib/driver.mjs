...
export async function launchBrowser(opts = {}) {
  ...
  // 실행 파일을 직접 지정할 수 있게 (playwright 번들 버전과 미리 깔린 크로미엄이 어긋나는
  // 컨테이너에서 다운로드 없이 돌리기 위한 탈출구 — 지정하면 채널 선택보다 우선한다)
  if (process.env.PW_EXECUTABLE) return chromium.launch({ ...o, executablePath: process.env.PW_EXECUTABLE });
